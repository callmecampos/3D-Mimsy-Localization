                            ...
                            try:
                                ...
                                if   self.inputBuf==[ord('R')]:
                                    ...
                                    if self.LOCALIZATION_MASK == ''.join(chr(i) for i in self.inputBuf[-12:]):
                                        ...
                                        # FIXME: indices are most likely wrong, ask Brian or Craig
                                        ...
                                        for i in range(0, pulse_track_count):
                                            offset = -i*8
                                            start = struct.unpack('<I',''.join([chr(b) for b in self.inputBuf[-16+offset:-12+offset]]))[0]
                                            end = struct.unpack('<I',''.join([chr(b) for b in self.inputBuf[-20+offset:-16+offset]]))[0]
                                            pulses.append((start, end))

                                            last_ind = -20+offset

                                        theta = struct.unpack('<f',''.join([chr(b) for b in self.inputBuf[last_ind-4:last_ind]]))[0]
