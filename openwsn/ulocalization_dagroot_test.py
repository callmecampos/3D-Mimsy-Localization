                        ...
                        if      (
                            ...
                            try:
                                ...
                                if self.inputBuf==[ord('R')]:
                                    ...
                                    if self.LOCALIZATION_MASK == ''.join(chr(i) for i in self.inputBuf[-12:]):
                                        message = self.inputBuf
                                        print(message[-28:-24])
                                        pulse_track_count = 5
                                        ...
                                        for i in range(0, pulse_track_count):
                                            offset = -i*8
                                            end = struct.unpack('<I',''.join([chr(c) for c in message[-28+offset:-24+offset]]))[0]
                                            start = struct.unpack('<I',''.join([chr(c) for c in message[-32+offset:-28+offset]]))[0]
                                            built_pulses.append((start, end))
                                        built_pulses = list(reversed(built_pulses))

                                        z = struct.unpack('<f',''.join([chr(c) for c in message[-16:-12]]))[0]
                                        y = struct.unpack('<f',''.join([chr(c) for c in message[-20:-16]]))[0]
                                        x = struct.unpack('<f',''.join([chr(c) for c in message[-24:-20]]))[0]
